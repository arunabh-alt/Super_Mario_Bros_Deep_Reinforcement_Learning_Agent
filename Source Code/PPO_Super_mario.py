    ...
    # It utilizes the Stable Baselines3 library, specifically the Proximal Policy Optimization (PPO) algorithm.
    # Command-line arguments are used to specify whether to train or test the agent, the learning algorithm (in this case, PPO), and optionally, a seed number.
    ...
    # A PPO agent is created with custom hyperparameters.
...
#  To run the code use this command line - python  PPO_Super_Mario.py (test|train) PPO <seed_number>
...
from stable_baselines3 import PPO # Importing PPO algorithm from stable_baselines3 library
...
from gym_super_mario_bros.actions import SIMPLE_MOVEMENT  # Importing SIMPLE_MOVEMENT action set from gym_super_mario_bros.actions for restricting actions.
...
if len(sys.argv) < 2 or len(sys.argv) > 4:
    print("USAGE: sb-SuperMarioBros2-v1.py (train|test) PPO [seed_number]")
    exit(0)
...
# Create the PPO agent with custom hyperparameters
